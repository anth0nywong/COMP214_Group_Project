...
export function DisplayFlightsPage (req: express.Request, res: express.Response, next: express.NextFunction)
{
    ...
    oracledb.getConnection(
        ...
        async (err, connection) => {
          ...
          { outFormat: oracledb.OUT_FORMAT_OBJECT });
          var availabileSeat = [econ.rows, business.rows, first.rows];
          console.log(availabileSeat);
            res.render('index', {title: 'Search', page: 'flight-search', results: result, date: req.body.date, basket: basket, user: user, availabileSeat: availabileSeat  });
