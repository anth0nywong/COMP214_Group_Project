        ...
        async (err, connection) => {
          ...
          SELECT COUNT(t.id_ticket) COUNT FROM pj_customers c ,pj_orders o, pj_ticket t
            ...
            if(result.rows)
            {
              ...
              SELECT COUNT(s.id_ticket) FROM TABLE (check_seat(:flightid)) s , pj_ticket p where s.id_ticket = p.id_ticket AND p.class = 'Economy'`,   //function for available seat
              {
                flightid: result.rows[i].ID_FLIGHT
              },
              { outFormat: oracledb.OUT_FORMAT_OBJECT });
              const business:oracledb.Result<any> = await connection.execute(`
              SELECT COUNT(s.id_ticket) FROM TABLE (check_seat(:flightid)) s , pj_ticket p where s.id_ticket = p.id_ticket AND p.class = 'Business'`,  //function for available seat
              {
                flightid: result.rows[i].ID_FLIGHT
              },
              { outFormat: oracledb.OUT_FORMAT_OBJECT });
              const first:oracledb.Result<any> = await connection.execute(`
              SELECT COUNT(s.id_ticket) FROM TABLE (check_seat(:flightid)) s , pj_ticket p where s.id_ticket = p.id_ticket AND p.class = 'First'`,      //function for available seat
              {
                flightid: result.rows[i].ID_FLIGHT
              },
              { outFormat: oracledb.OUT_FORMAT_OBJECT });
              ...
              if(econ.rows)
              {
                result.rows[i].availabileEcon = econ.rows[0];
              }
              if(business.rows)
              {
                result.rows[i].availabileBusiness = business.rows[0];
              }
              if(first.rows)
              {
              result.rows[i].availabileFirst = first.rows[0];
              }              
