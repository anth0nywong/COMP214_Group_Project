  ...
  oracledb.getConnection(
    ...
    async (err, connection) => {
      ...
      WHERE availability = 'Y' AND class = :class AND flightid = :flightid AND orderid IS NULL`, 
          ...
          SELECT COUNT(s.id_ticket) FROM TABLE (check_seat(:flightid)) s , pj_ticket p where s.id_ticket = p.id_ticket AND p.class = 'Economy'`, 
          {
            flightid: req.params.flightId
          },
          { outFormat: oracledb.OUT_FORMAT_OBJECT });
          const business:oracledb.Result<any> = await connection.execute(`
          SELECT COUNT(s.id_ticket) FROM TABLE (check_seat(:flightid)) s , pj_ticket p where s.id_ticket = p.id_ticket AND p.class = 'Business'`, 
          {
            flightid: req.params.flightId
          },
          { outFormat: oracledb.OUT_FORMAT_OBJECT });
          const first:oracledb.Result<any> = await connection.execute(`
          SELECT COUNT(s.id_ticket) FROM TABLE (check_seat(:flightid)) s , pj_ticket p where s.id_ticket = p.id_ticket AND p.class = 'First'`, 
          {
            flightid: req.params.flightId
          },
          { outFormat: oracledb.OUT_FORMAT_OBJECT });
          var availabileSeat = [econ, business, first];
          console.log(availabileSeat)
          // basket
          ...
          SELECT COUNT(t.id_ticket) COUNT FROM pj_customers c ,pj_orders o, pj_ticket t
            ...
            { outFormat: oracledb.OUT_FORMAT_OBJECT });
          await connection.close();
          res.render('index', {title: 'Select Ticket', flight: flight, page: 'select', ticket: ticket, basket: basket, user: user, flight_id: req.params.flightId, availabileSeat: availabileSeat });
    });
}
