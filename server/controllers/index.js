...
function DisplayHomePage(req, res, next) {
    oracledb_1.default.getConnection({
        ...
        if (err) {
            ...
            await console.log("Succesfully Login Oracle Database with user" + DBConfig.user);
        }
        const user = await connection.execute(`
          SELECT * FROM PJ_CUSTOMERS WHERE id_user = :userNum`, {
            userNum: userId,
        }, { outFormat: oracledb_1.default.OUT_FORMAT_OBJECT });
        const basket = await connection.execute(`SELECT COUNT(t.id_ticket) COUNT FROM pj_customers c ,pj_orders o, pj_ticket t
