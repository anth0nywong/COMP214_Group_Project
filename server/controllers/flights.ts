  ...
  oracledb.getConnection(
    ...
    async (err, connection) => {
      if (err) {
        ...
        console.log(
          "Succesfully Login Oracle Database with user" + DBConfig.user
        );
      }
      // select airport information
      const result: oracledb.Result<any> = await connection.execute(
        //oracle db function(query, parameters, output format config)
      ...
      if (result.rows) {
        for (let i = 0; i < result.rows.length; i++) {
          const minPrice: oracledb.Result<any> = await connection.execute(
            //minimum price procedure with OUT parameters
            `BEGIN
...
export function DisplaySelectPage(
  req: express.Request,
  res: express.Response,
  next: express.NextFunction
) {
...
export function DisplayCheckoutTable(
  req: express.Request,
  res: express.Response,
  next: express.NextFunction
) {
